function [solk0] = fun_Vacuum_FRIDA_evol(meshData,meshData_loc,Conductors,SETTINGS)
...
if SETTINGS.VAC_METHOD == 1 %
    ...
    error('SETTINGS.VAC_METHOD == 1 not available in evol mode')
...
% %     hh = waitbar(0, 'computing Vacuum solution ...');
    ...
    Psi_MS_bc_act = zeros(size(Points_bc,1),1);
...
% %         plot(nodes(:,1),nodes(:,2),'o')
        ...
        if SETTINGS.RUN_MEX_ROUTINE == false
            ...
            Psi_MS_bc_act_ii = fun_Green_Flux_Loop( source, point, j_source);
            Psi_MS_bc_act_ii = Psi_MS_bc_act_ii/2/pi;
        ...
        elseif SETTINGS.RUN_MEX_ROUTINE == true
            ...
            psi = fun_Green_filament_flux_SP_f90(npt_source,R_source,Z_source,I_source,npt_point,R_points,Z_points,OPT_PARALLEL,n_threads);
            Psi_MS_bc_act_ii = psi/2/pi;

        end
   
        Psi_MS_bc_act = Psi_MS_bc_act + Psi_MS_bc_act_ii;        
        
    end
% %     close(hh)
    
    % Passive current contribution (previous time step)
% %     GG_pas_bc = meshData.Green_pas_Bc/2/pi;
% %     Psi_MS_bc_pas = GG_pas_bc*SETTINGS.i_pas_old;
    Psi_MS_bc_pas = SETTINGS.psi_bc_pas_old;
    
    Psi_MS_bc = Psi_MS_bc_act + Psi_MS_bc_pas;
...
% %         save temp Psi_MS Psi_MS_bc_act
% %         qq = load('temp');

% %     end
    
    solk0.Psi_MS_bc_act = Psi_MS_bc_act;
    solk0.Psi_MS_bc_pas = Psi_MS_bc_pas;
...
% %             set(edges,'color','k')
% %             set(edges,'linewidth',1)
% %             rr=meshData_loc.n(:,1); zz=meshData_loc.n(:,2);
% %             axis equal;
% %             hold on, xlabel('r [m]'), ylabel('z [m]');
% %             rgrid=linspace(min(rr),max(rr),200);
% %             zgrid=linspace(min(zz),max(zz),200);
% %             [RR,ZZ] = meshgrid(rgrid,zgrid);
% %             PSI = griddata(rr,zz,Psi_MS-Psi_MS2,RR,ZZ);
% %             contour(RR,ZZ,PSI,200);
% %             axis([min(rr) max(rr) min(zz) max(zz)]),
