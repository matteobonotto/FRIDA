function [solk1]=fun_NR_FRIDA_evol_BackwardEuler_fast(INPUT_solver)
...
% %     solk.lambda_star = solk.lambda_star;
...
%
b_phic = A_phic_inv*(solk.phi_pas_old + h_step*B_act*I_act);
...
b_psi_bc_act = solk.Psi_MS_bc_act + b_psic_bc; % b_psic_bc = RHS_psic_bc
...
% % else
% %     figure
% %     subplot(1,3,1); plot(solk.phi_pas,'k'); hold on; plot(phi_pas,'r--'); title('psi pas')
% %     subplot(1,3,2); plot(solk.psi_bc_pas,'k'); hold on; plot(psi_bc_pas,'r--'); title('psi bc pas')
% %     subplot(1,3,3); plot(KONNAX_passive'*solk.i_pas,'k'); hold on; plot(KONNAX_passive'*i_pas,'r--'); title('i pas')
% % end
...
solk1.i_pas_old      = solk.i_pas_old;
solk1.psi_bc_pas_old = solk.psi_bc_pas_old;
