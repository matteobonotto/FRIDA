...
! fortran output
    real*8, allocatable, dimension(:) :: vec_Aphi


!                    i1         i2       i3       i4       i5        i6      i7      i8           i9       o1
!call fun_Green_Aphi(npt_source,r_source,z_source,I_source,npt_point,r_point,z_point,OPT_PARALLEL,n_thread,vec_Aphi)


! check for proper number of arguments.
      if (nrhs .ne. 9) then
        call mexErrMsgIdAndTxt ('MATLAB:fun_Green_Aphi:nInput', &
...
! call the computational subroutine
        allocate(vec_Aphi(npt_point))
        call fun_Green_filament_Aphi_SP_4mex(npt_source,r_source,z_source,I_source, &
                            npt_point,r_point,z_point,OPT_PARALLEL,n_thread,vec_Aphi)
...
! Load the output into a MATLAB array.
      vec_Aphi_pr = mxGetPr(plhs(1))
      size=mo*no
      call mxCopyReal8ToPtr(vec_Aphi, vec_Aphi_pr, size)
...
!------------------------------------------------------------------------------------------------------------------
    subroutine fun_Green_filament_Aphi_SP_4mex(npt_source,r_source,z_source,I_source, &
                              npt_point,r_point,z_point,OPT_PARALLEL,n_thread,vec_Aphi)
...
!-
    ...
    real(kind=8) r_point(npt_point), z_point(npt_point), vec_Aphi(npt_source)

!-
    if(OPT_PARALLEL == 0) then
        call sub_Green_Aphi_serial(npt_source,r_source,z_source,I_source,npt_point,r_point,z_point,vec_Aphi)

    else if(OPT_PARALLEL == 1) then
        call sub_Green_Aphi_parallel(npt_source,r_source,z_source,I_source,npt_point,r_point,z_point,n_thread,vec_Aphi)

    endif


    end subroutine fun_Green_filament_Aphi_SP_4mex
!------------------------------------------------------------------------------------------------------------------
