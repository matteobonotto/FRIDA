function [ind_rows,ind_cols,dSource_Dpsi_vals] = fun_dSource_Dpsi_type_2(...
    triangles,...
    N_order,...
    dim_ind_rows,...
    P_Gauss, ...
...
if N_order == 1
    ...
    for ii = 1:length(ind_n_INpla)
        ...
        for jj = 1:size(tri,1)
            ...
            aa_r_tot_ii = aa_r_tot(ii_tri_Gauss);
            qq1 = power(complex(psi_bar_ii(:)),alpha_M);
            qq2 = (1-qq1);
            tmp_gg_pert_psi = power(complex(qq2),alpha_N);
            tmp_gg_pert_psi(imag(tmp_gg_pert_psi) ~= 0) = abs(tmp_gg_pert_psi(imag(tmp_gg_pert_psi) ~= 0)); % occhio!!!
            tmp_gg_pert_psi = real(tmp_gg_pert_psi);
            gg_pert_psi = tmp_gg_pert_psi.*aa_r_tot_ii;
