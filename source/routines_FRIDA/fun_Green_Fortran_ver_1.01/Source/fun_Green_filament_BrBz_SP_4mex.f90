...
! fortran output    
    real*8, allocatable, dimension(:) :: vec_Br, vec_Bz


! EXAMPLE                             i1         i2       i3       i4       
!call fun_Green_filament_BrBz_SP_4mex(npt_source,r_source,z_source,I_source, 
!                       npt_point,r_point,z_point,OPT_PARALLEL,n_thread,vec_Br,vec_Bz)
!                       i5        i6      i7      i8           i9       o1     o2
...
! call the computational subroutine
        allocate(vec_Br(npt_point), vec_Bz(npt_point))
        call fun_Green_filament_BrBz_SP_4mex(npt_source,r_source,z_source,I_source, &
                            npt_point,r_point,z_point,OPT_PARALLEL,n_thread,vec_Br,vec_Bz)
...
! Load the output into a MATLAB array.
      vec_Br_pr = mxGetPr(plhs(1))
      size=mo*no
      call mxCopyReal8ToPtr(vec_Br, vec_Br_pr, size)


! Create an array for the #2 return argument
      mo=npt_point
      no=1
      plhs(2) = mxCreateDoubleMatrix(mo, no, 0)
! Load the output into a MATLAB array.
      vec_Bz_pr = mxGetPr(plhs(2))
      size=mo*no
      call mxCopyReal8ToPtr(vec_Bz, vec_Bz_pr, size)

      deallocate(r_source,z_source,I_source,r_point,z_point)
	  deallocate(vec_Br,vec_Bz)
...
!------------------------------------------------------------------------------------------------------------------
    subroutine fun_Green_filament_BrBz_SP_4mex(npt_source,r_source,z_source,I_source, &
                              npt_point,r_point,z_point,OPT_PARALLEL,n_thread,vec_Br,vec_Bz)
...
!-
    if(OPT_PARALLEL == 0) then
        call sub_Green_BrBz_serial(npt_source,r_source,z_source,I_source, & 
                                   npt_point,r_point,z_point,vec_Br,vec_Bz)

    else if(OPT_PARALLEL == 1) then
        call sub_Green_BrBz_parallel(npt_source,r_source,z_source,I_source, & 
                                      npt_point,r_point,z_point,n_thread,vec_Br,vec_Bz)

    endif


    end subroutine fun_Green_filament_BrBz_SP_4mex
!------------------------------------------------------------------------------------------------------------------
