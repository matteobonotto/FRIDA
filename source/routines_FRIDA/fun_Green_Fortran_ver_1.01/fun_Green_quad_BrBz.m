function [out_Br,out_Bz] = fun_Green_quad_BrBz(R_source_quad,Z_source_quad,j_source,R_points,Z_points,n_Gauss_quad,OPT_PARALLEL)
...
if size(R_source_quad,2) == 1 % only 1 coil
    ...
    out_Br = zeros(length(R_points),1);
    out_Bz = zeros(length(R_points),1);
    ...
    for ii=1:n_coils
        ...
        if OPT_PARALLEL == 'F'
            [out_Br_ii,out_Bz_ii] = fun_Green_filament_BrBz_SP_f90(npt_source,R_source,Z_source, ...
                I_source,npt_point,R_points,Z_points,0,n_threads);
        elseif OPT_PARALLEL == 'T'
            [out_Br_ii,out_Bz_ii] = fun_Green_filament_BrBz_SP_f90(npt_source,R_source,Z_source, ...
                I_source,npt_point,R_points,Z_points,1,n_threads);
        ...
        out_Br = out_Br + out_Br_ii;
        out_Bz = out_Bz + out_Bz_ii;
